...
{
    public record GameRatingInfo(Guid GameId, double AverageRating, int TotalReviews);

    public class GameRatingInfoProjection : SingleStreamProjection<GameRatingInfo>
    {
        public static GameRatingInfo Create(GameCreated gameCreatedEvent)
        {
            return new GameRatingInfo(
                GameId: gameCreatedEvent.AggregateId,
                AverageRating: 0,
                TotalReviews: 0);
        }
